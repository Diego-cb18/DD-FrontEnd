...
function ReportCard({ report, onViewVideo }) {
  const {
    first_name,
    last_name,
    dni,
  ...
  const getMostSevereEvent = () => {
    ...
    const sorted = [...critical_events].sort((a, b) => {
      const aKey = Object.keys(severities).find(k => a.toLowerCase().includes(k)) || "";
      const bKey = Object.keys(severities).find(k => b.toLowerCase().includes(k)) || "";
  ...
  if (tipo.includes("minibús") || tipo.includes("minibus")) return minibusDefault;
  return null;
 };
    ...
    <div className="bg-white/40 rounded-2xl shadow-[0_10px_15px_-3px_rgba(0,0,0,0.4)] p-8 w-2/3 max-w-6xl mx-auto mb-6 grid grid-cols-[1.3fr_1.3fr_2.5fr] gap-6">
      ...
      <div className="text-[15px] font-semibold space-y-1">
        <div className="flex justify-center">
          <img src={userDefault} alt="Foto conductor" className="w-25 h-25 rounded-full mb-4 mr-8 object-cover" />
        </div>
      ...
      {/* Columna 2: Vehículo */}
        <div className="text-[15px] font-semibold space-y-1">
            <div className="flex justify-center">
                <img
                    ...
                    vehicle_type?.toLowerCase().includes("minibús") || vehicle_type?.toLowerCase().includes("minibus")
                        ? "w-28 h-20"
                        : "w-36 h-20"
                    }`}
                />
        ...
        <div className="gap-y-1">
           {(() => {
            ...
            return (
                <>
                <p>Peso: {weight ?? defaults.weight} - Altura: {height ?? defaults.height}</p>
                </>
            );
            })()}
        </div>
      ...
      {/* Columna 3: Reporte */}
        ...
        <div className="space-y-1">
            <p className="text-center text-lg font-bold mb-2 mr-2 py-2">REPORTE:</p>
            ...
            <div className="mt-6 flex justify-center w-full">
              <button
                onClick={onViewVideo} 
                className="bg-[#0C0C0C] text-white px-4 py-2 rounded border hover:bg-gray-700 text-sm cursor-pointer"
              >
                Ver video
              </button>
            </div>
